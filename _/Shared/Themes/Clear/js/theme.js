...
$(document).on('click', ".layout-grid.active, .layout-list.active", function(e){
  e.preventDefault();
  return false;
});


$(document).on('click', ".layout-grid:not(.active), .layout-list:not(.active)", function(e){
  e.preventDefault();
  $('.layout-grid, .layout-list').toggleClass('active');
  $('.small-product').toggleClass('col-md-4 col-sm-6 grid-view list-view col-xs-12');
});
